...
torch.manual_seed(0)
logging.basicConfig(filename='training.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
def train_model(dataset_name):
    ...
    unique_labels, counts_train_label = torch.unique(train_label_tensor, return_counts=True)
    unique_labels, counts_val_label = torch.unique(valid_label_tensor, return_counts=True)
    criterion = CBLoss([counts_train_label[0],counts_train_label[1]], 2)
    criterion2 = CBLoss([counts_val_label[0],counts_val_label[1]], 2)
    ...
    for epoch in tqdm(range(current_checkpoint_num, num_epochs+1)):
        ...
        with torch.no_grad():
            ...
            for inputs, labels, word_edge_index, line_edge_index in batch_generator(valid_code_vec_pad, valid_label_tensor, valid_padded_edge, valid_line_edge, batch_size):
                ...
                val_loss = criterion2(output, labels.reshape(batch_size,1))
