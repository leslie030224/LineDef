...
import logging
max_seq_len = 50
logging.basicConfig(filename='util.log', filemode='w', level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
class CBLoss(nn.Module):
    def __init__(self, samples_per_cls, no_of_classes, loss_type='sigmoid', beta=0.9999, gamma=2):
        super(CBLoss, self).__init__()
        self.loss_type = loss_type
        self.beta = beta
        self.gamma = gamma
        effective_num = 1.0 - torch.pow(self.beta, torch.tensor(samples_per_cls))
    ...
    def forward(self, inputs, targets):
        weights = self.weights[targets.to(torch.long)]
        cb_loss = F.binary_cross_entropy(inputs, targets, reduction='none')
        cb_loss = weights * cb_loss
        # logging.info("cb_loss:{}".format(cb_loss))
