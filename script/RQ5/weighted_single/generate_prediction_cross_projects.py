...
def predict_defective_files_in_releases(dataset_name, target_epochs):
    ...
    if exp_name == '':
        ...
        checkpoint = torch.load(
            actual_save_model_dir + exp_name + '/checkpoint_' + exp_name + '_' + target_epochs + 'epochs.pth',
            map_location=device)
        intermediate_output_dir = '../output/intermediate_output/LineDef/cross-project/' + exp_name
    ...
    for rel in test_rel:
        ...
        for filename, df in tqdm(test_df.groupby('filename')):
            ...
            code2d, word_adj, line_adj, word_edge_weight, line_edge_weight = prepare_code2d(code, line_lenth, True, weighted_graph)
            ...
            line_edge_weight = [line_edge_weight]
            ...
            with torch.no_grad():
                ...
                output, word_att_weights, line_att_weight, _ = model(codevec_padded_tensor, word_adj, line_adj, word_edge_weight, line_edge_weight)
