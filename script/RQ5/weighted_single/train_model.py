...
arg.add_argument('-weighted_graph', type=bool, default=True, help='Whether to use weighted graph')
...
def train_model(dataset_name):
    ...
    train_code3d, train_label, train_word_edge, tarin_line_edge, train_word_weight, train_line_weight = get_code3d_and_label(
        train_df, True)
    valid_code3d, valid_label, valid_word_edge, valid_line_edge, valid_word_weight, valid_line_weight = get_code3d_and_label(
        valid_df, True)
    ...
    train_padded_edge, tarin_pad_word_weight = pad_word_edge_index(train_word_edge, max_sent_len, train_word_weight)
    tarin_line_edge, tarin_pad_line_weight = pad_line_edge_index(tarin_line_edge, max_sent_len, train_line_weight)
    ...
    valid_padded_edge, val_pad_word_weight = pad_word_edge_index(valid_word_edge, max_sent_len, valid_word_weight)
    valid_line_edge, val_pad_line_weight = pad_line_edge_index(valid_line_edge, max_sent_len, valid_line_weight)
    ...
    if total_checkpoints == 0:
        ...
        checkpoint = torch.load(
            actual_save_model_dir + 'checkpoint_' + exp_name + '_' + str(current_checkpoint_num) + 'epochs.pth')
    ...
    for epoch in tqdm(range(current_checkpoint_num, num_epochs + 1)):
        ...
        for inputs, labels, word_edge_index, line_edge_index, word_weight, line_weight in batch_generator(
                tarin_code_vec_pad, train_label_tensor,
                train_padded_edge, tarin_line_edge,
                tarin_pad_word_weight, tarin_pad_line_weight,
                batch_size):
            output, _, __, ___ = model(inputs, word_edge_index, line_edge_index, word_weight, line_weight)
        ...
        with torch.no_grad():
            ...
            for inputs, labels, word_edge_index, line_edge_index, word_weight, line_weight in batch_generator(valid_code_vec_pad,
                                                                                    valid_label_tensor,
                                                                                    valid_padded_edge, valid_line_edge,
                                                                                    val_pad_word_weight, val_pad_line_weight,
                                                                                    batch_size):
                output, _, __, ___ = model(inputs, word_edge_index, line_edge_index, word_weight, line_weight)
