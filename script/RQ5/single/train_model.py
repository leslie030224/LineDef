...
def train_model(dataset_name):
    ...
    train_code3d, train_label, train_word_edge, train_line_edge = get_code3d_and_label(train_df, True)
    ...
    train_code_vec_pad = torch.tensor(pad_code(x_train_vec, max_sent_len))
    train_word_edge = pad_word_edge_index(train_word_edge, max_sent_len)
    train_line_edge = pad_line_edge_index(train_line_edge, max_sent_len)
    ...
    valid_word_edge = pad_word_edge_index(valid_word_edge, max_sent_len)
    ...
    for epoch in tqdm(range(current_checkpoint_num, num_epochs+1)):
        ...
        for inputs, labels, word_edge_index, line_edge_index in batch_generator(train_code_vec_pad, train_label_tensor, train_word_edge, train_line_edge, batch_size):
        ...
        with torch.no_grad():
            ...
            for inputs, labels, word_edge_index, line_edge_index in batch_generator(valid_code_vec_pad, valid_label_tensor, valid_word_edge, valid_line_edge, batch_size):
    ...
    df_results.to_csv(f'../output/{exp_name}_{dataset_name}_put.csv', index=False)
