...
def batch_generator(code, label, word_edge, line_edge, word_weight, line_weight, batch_size, random_seed=0):
    ...
    for i in range(num_batches):
        ...
        batch_line_edge_weight = [word_weight[idx] for idx in batch_indices]
        batch_word_edge_weight = [line_weight[idx] for idx in batch_indices]

        yield batch_code, batch_label, batch_word_edge, batch_line_edge, batch_line_edge_weight, batch_word_edge_weight


def pad_word_edge_index(line_egde_index_list, max_sent_len, word_weight, limit_sent_len=True):
    padded_edge_list = []
    padded_weight_list = []
    for weight, edge in zip(word_weight, line_egde_index_list):
        num = max_sent_len - len(edge)
        if max_sent_len - len(edge) > 0 :
            for _ in range(num):
                edge.append(np.full((2, 1), -1))
                # weight.append(np.array(-1))

        if limit_sent_len == True:
            padded_edge_list.append(edge)
            padded_weight_list.append(weight)
        else:
            padded_edge_list.append(edge)
            padded_weight_list.append(weight)

    return padded_edge_list, padded_weight_list


def pad_line_edge_index(line_egde_index_list, max_sent_len, line_weight, limit_sent_len=True):
    paded_line_egde = []
    paded_line_weight = []
    if limit_sent_len==True:
        for edge_index, weight in zip(line_egde_index_list, line_weight):
            ...
            weight = np.delete(weight, column_numbers, axis=0)
            paded_line_egde.append(edge_index)
            paded_line_weight.append(weight)
    return paded_line_egde, paded_line_weight
...
def prepare_line_adj(line_lenth, weighted_graph = True):
    ...
    col = []
    weight = []
    # logging.info("line_word_pair_count: {}".format(word_pair_count))
    ...
    edge_weight_matrix = np.array(weight)
    return edge_index, edge_weight_matrix
...
def prepare_code2d(code_list, line_lenth, to_lowercase=False, weighted_graph=False):
    ...
    for c in code_list:
        ...
        col = []
        weight = []
        # logging.info("word_word_pair_count: {}".format(word_pair_count))
        for key in word_pair_count:
            p = key[0]
            q = key[1]
            row.append(p)
        ...
        all_word_edge_weight.append(np.array(weight))

    line_all_edge_index, line_edge_weight = prepare_line_adj(line_lenth, True)

    return code2d, all_word_edge_index, line_all_edge_index, all_word_edge_weight, line_edge_weight
...
def get_code3d_and_label(df, to_lowercase=False, weighted_graph=False):
    ...
    for filename, group_df in df.groupby('filename'):
        ...
        code2d, word_adj, line_adj, word_weight, line_weight = prepare_code2d(code, line_lenth, to_lowercase, weighted_graph)
        ...
        word_weight3d.append(word_weight)
        line_weight3d.append(line_weight)
    return code3d, all_file_label, word_adj3d, line_adj3d, word_weight3d, line_weight3d
