...
class HierarchicalAttentionNetwork(nn.Module):
    ...
    def forward(self, code_tensor, word_edge, line_edge, word_weight, line_weight):
        # 此时传入的是一个文件file的代码tensor，相当于把同一个文件的code行给做了成了一个list
        code_lengths = []
        sent_lengths = []
        ...
        code_embeds, word_att_weights, sent_att_weights, sents = self.sent_attention(code_tensor, code_lengths,
                                                                                     word_edge, line_edge, line_weight)
...
class SentenceAttention(nn.Module):
    ...
    def forward(self, code_tensor, code_lengths, word_edge, line_edge, line_edge_weight):
        ...
        for node, edge, weight in zip(sents, line_edge, line_edge_weight):
            edge = torch.tensor(edge, dtype=torch.long).to(self.device)
            weight = torch.tensor(weight, dtype=torch.float).to(self.device)
            line_data_list.append(Data(node, edge, edge_weight=weight))  # node.shape [50, 50]  adj.shape [2, E]

        batch = Batch.from_data_list(line_data_list)
        line_out = self.gcn(batch.x, batch.edge_index, edge_weight=batch.edge_weight)
